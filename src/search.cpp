#include "search.hpp"
#include "datatypes.hpp"
#include <chrono>
#include <limits.h>

Search::Search(Position *position_ptr)
    : move_gen(position_ptr), eval(position_ptr) {
  pos = position_ptr;
  search_start = std::chrono::high_resolution_clock::now();
  search_done = false;
  nodes_searched = 0;
  best_move = {};
  hashfull = 0;
  hashsize = 250 * 1024 * 1024 / sizeof(TT_Entry);

  transposition_table.reserve(hashsize);
  for (zobrist_key i = 0; i < (hashsize); i++) {
    TT_Entry entry = TT_Entry();
    transposition_table.push_back(entry);
  }
}

int Search::iterative_deepening(int time, int moves_remaining) {

  int depth = 1;
  int eval = 0;

  time_limit = 0.75 * time / moves_remaining;
  search_start = std::chrono::high_resolution_clock::now();
  search_done = false;

  while (!search_done) {
    auto start_time = std::chrono::high_resolution_clock::now();

    eval = negamax_root(depth);
    depth++;
    if (!search_done){
    best_move_overall = best_move;
    }

    auto end_time = std::chrono::high_resolution_clock::now();
    time_searched = std::chrono::duration_cast<std::chrono::milliseconds>(
                        end_time - start_time)
                        .count();
    time_searched = (time_searched > 0) ? time_searched : 1;
    double nps = nodes_searched / (time_searched / 1000.0);

    std::cout << "info depth " << depth_searched << " score cp " << eval / 10
              << " nodes " << nodes_searched << " nps " << nps << " hashfull "
              << hashfull * 1000 / hashsize << " time " << time_searched
              << " pv " << best_move_overall << std::endl;
  }

  return eval;
}

bool Search::update_TT(zobrist_key z_key, size_t depth, int evaluation,
                       bool exact, bool upper_bound, bool lower_bound,
                       Move best_move) {
  zobrist_key idx = z_key % transposition_table.size();

  if (depth > transposition_table[idx].depth) {
    TT_Entry entry = TT_Entry(z_key, depth, evaluation, exact, upper_bound,
                              lower_bound, best_move);
    transposition_table[idx] = entry;
    hashfull++;
    return true;
  } else {
    return false;
  }
}

TT_Entry Search::probe_TT(zobrist_key z_key, size_t depth, bool &was_found) {

  zobrist_key idx = z_key % transposition_table.size();
  TT_Entry entry = transposition_table[idx];

  if ((entry.key == z_key) && (entry.depth > depth)) {
    was_found = true;
  } else {
    was_found = false;
  }

  return entry;
}

int Search::negamax_root(int depth) {

    best_move = {};
  int alpha = -INT_MAX;
  int beta = INT_MAX;
  int eval = -INT_MAX;
  depth_searched = depth;

  MoveList moves = move_gen.generate_legal_moves();

  bool was_found = false;
  TT_Entry entry = probe_TT(pos->z_key, pos->ply, was_found);
  if (was_found && entry.exact == true && !(entry.best_move == Move())) {
    best_move = entry.best_move;
    alpha = entry.evaluation;
  }

  moves.score_moves(entry.best_move);
  moves.sort_moves();

  zobrist_key move_key = pos->z_key;
  size_t move_depth = pos->ply;

  for (int i = 0; i < moves.size(); i++) {
    Move mv = moves.at(i);

    pos->make_move(mv);
    nodes_searched++;
    // recursive call

    eval = -negamax(-beta, -alpha, depth - 1);

    // if a recursive search returns a stronger move
    // we save the parent of that subtree as best
    if (eval > alpha) {
      alpha = eval;
      best_move = moves.at(i);
    }

    // reset board before making different move
    pos->undo_move(mv);
  }

  update_TT(move_key, move_depth, eval, false, false, true, best_move);

  return alpha;
}

int Search::negamax(int alpha, int beta, int depth) {

  int alpha_old = alpha;

  if (nodes_searched & 1023) {
    auto curr_time = std::chrono::high_resolution_clock::now();
    auto search_duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(curr_time -
                                                              search_start);
    if (search_duration.count() > time_limit) {
      search_done = true;
    }
  }

  if ((depth <= 0) || search_done) {
    return quiescence(alpha, beta);
  }

  bool was_found = false;
  TT_Entry entry = probe_TT(pos->z_key, pos->ply, was_found);
  if (was_found) {
    if (entry.exact) {
      return entry.evaluation;
    } else if (entry.lower_bound && (entry.evaluation <= alpha)) {
      return alpha;
    } else if (entry.upper_bound && (entry.evaluation >= beta)) {
      return beta;
    }
  }

  int best_eval = -INT_MAX;
  int eval = -INT_MAX;

  MoveList moves = move_gen.generate_legal_moves();
  moves.score_moves(entry.best_move);
  moves.sort_moves();

  if (moves.empty()) {
    if (move_gen.king_in_check(pos->side_to_play)) {
      return -INT_MAX + pos->ply;
    } else {
      return 0;
    }
  }

  for (int i = 0; i < moves.size(); i++) {
    // make move on board
    Move mv = moves.at(i);

    pos->make_move(mv);
    nodes_searched++;
    zobrist_key move_key = pos->z_key;
    size_t move_depth = pos->ply;
    // recursive call

    eval = -negamax(-beta, -alpha, depth - 1);

    // undo to reset board
    pos->undo_move(mv);

    if (eval >= beta) {
      update_TT(move_key, move_depth, eval, false, false, true, best_move);
      return beta;
    }

    if (eval > best_eval) {
      best_eval = eval;
      if (eval > alpha) {
        alpha = eval;
      }
    }

    if (alpha > alpha_old) {
      update_TT(move_key, move_depth, best_eval, true, false, false, best_move);
    } else {
      update_TT(move_key, move_depth, alpha, false, true, false, best_move);
    }
  }
  return alpha;
}

int Search::quiescence(int alpha, int beta) {
  int stand_pat = eval.evaluate();

  if (stand_pat >= beta) {
    return beta;
  }
  if (alpha < stand_pat) {
    alpha = stand_pat;
  }

  int eval = stand_pat;

  MoveList moves = move_gen.generate_legal_moves();
  moves.score_moves(Move());
  moves.sort_moves();

  if (moves.empty()) {
    if (move_gen.king_in_check(pos->side_to_play)) {
      return -INT_MAX + pos->ply;
    } else {
      return 0;
    }
  }
  for (int i = 0; i < moves.size(); i++) {
    Move mv = moves.at(i);
    if (!mv.is_capture) {
      continue;
    }

    pos->make_move(mv);
    nodes_searched++;
    eval = -quiescence(-beta, -alpha);
    pos->undo_move(mv);

    if (eval >= beta) {
      return beta;
    }

    if (eval > alpha) {
      alpha = eval;
    }
  }

  return alpha;
}
